...
void UI::update(ListOfObjects& list)
{
	...
	while (selectedAction != -1)
	{
		...
		case 1: 
		{
			std::cout << "Введите название файла для чтения с указанием формата из корневой директории" << std::endl 
				<< "(например, прилагаемый тестовый файл : input.txt) : " 
				<< std::endl;
			std::string filename;
			std::cin >> filename;
		...
		case 3:
		{
			...
			std::cout << "Данные сохранены в файле output.txt, для продолжения введите любое значение!" << std::endl;
			std::cin >> n;
			break;
		}
		case -1:
